...
class wordcloud {
    ...
    public function add_word($userid, $word, $groupid = 0) {
        global $DB;
        ...
        if (mb_strlen($word) > get_config('wordcloud', 'wordmaxlenght')) {
            return self::ERROR_WORD_TOO_LONG;
        }
        
        if ($DB->record_exists_sql('SELECT * FROM {wordcloud_words} ww WHERE ww.word = ? AND ww.wcid = ? AND ww.userid = ? AND ww.groupid = ?', array($word, $this->activity->id, $userid, $groupid))) {
    ...
    public function add_words($userid, array $words, $groupid = 0) {
        $return = true;
    ...
    public function get_user_words($userid, $groupid = 0) {
        global $DB;
        
        return $DB->get_records('wordcloud_words', array('wcid' => $this->activity->id, 'userid' => $userid, 'groupid' => $groupid));
    }
    
    public function rename_word($oldword, $newword, $groupid = 0) {
        global $DB;
        ...
        if (mb_strlen($newword) > get_config('wordcloud', 'wordmaxlenght')) {
            return self::ERROR_WORD_TOO_LONG;
        }
        
        $words = $DB->get_records_sql('SELECT id, userid, word FROM {wordcloud_words} ww WHERE ww.word = ? AND ww.wcid = ? AND ww.groupid = ?', array($oldword, $this->activity->id, $groupid));
        ...
        foreach ($words AS $word) {
            $new_words = $DB->get_records_sql('SELECT id, word FROM {wordcloud_words} ww WHERE ww.word = ? AND ww.wcid = ? AND ww.groupid = ? AND userid = ?', array($newword, $this->activity->id, $groupid, $word->userid));
    ...
    public function simulate_rename_word($oldword, $newword, $groupid = 0) {
        global $DB;
        ...
        $current_words = $DB->get_records_sql('SELECT id, userid, word FROM {wordcloud_words} ww WHERE ww.word = ? AND ww.wcid = ? AND ww.groupid = ?', array($newword, $this->activity->id, $groupid));
    ...
    public function remove_word($word, $groupid = 0) {
        global $DB;

        $words = $DB->get_records_sql('SELECT * FROM {wordcloud_words} ww WHERE ww.wcid = ? AND ww.groupid = ? AND ww.word = ?', array($this->activity->id, $groupid, $word));

        $DB->execute('DELETE FROM {wordcloud_words} WHERE wcid = ? AND groupid = ? AND word = ?', array($this->activity->id, $groupid, $word));
    ...
    public function get_cloud_words($groupid = 0) {
        global $DB;
        ...
        $words = $DB->get_records_sql('SELECT ww.word, count(id) AS nb, (SELECT LENGTH(word) FROM {wordcloud_words} ww2 WHERE ww2.wcid = ww.wcid AND ww2.groupid = ww.groupid ORDER BY LENGTH(word) DESC LIMIT 1) AS maxlen 
FROM {wordcloud_words} ww WHERE ww.wcid = ? AND ww.groupid = ? GROUP BY ww.word, ww.wcid, ww.groupid ORDER BY nb DESC LIMIT 250', array($this->activity->id, $groupid));
    ...
    public function get_cloud_word_info($word, $groupid = 0) {
        global $DB, $OUTPUT;
        
        $result = $DB->get_records_sql("SELECT ww.userid, ww.id, 
COALESCE(u.firstname,'Anonyme') AS firstname,
COALESCE(u.lastname,'Anonyme') AS lastname,
(SELECT COUNT(id) FROM {wordcloud_words} ww2 WHERE ww2.wcid = ww.wcid AND ww2.word = ww.word AND ww2.groupid = ww.groupid) AS nb
FROM {wordcloud_words} ww
LEFT JOIN {user} u ON (u.id = ww.userid)
WHERE ww.wcid = ?
AND ww.word = ?
AND ww.groupid = ?
GROUP BY ww.userid, ww.id, u.firstname, u.lastname, ww.wcid", array($this->activity->id, $word, $groupid));
    ...
    public function get_cloud_users($groupid = 0) {
        global $DB;

        $users = $DB->get_records_sql("SELECT ww.userid AS id, COALESCE(u.firstname,'Anonyme') AS firstname, COALESCE(u.lastname,'Anonyme') AS lastname 
FROM {wordcloud_words} ww LEFT JOIN {user} u ON (u.id = ww.userid) WHERE ww.wcid = ? AND ww.groupid = ? GROUP BY ww.userid, u.firstname, u.lastname", array($this->activity->id, $groupid));

        return $users;
    }
    
    public function get_cloud_users_words($groupid = 0) {
        global $DB;
        
        $words = $DB->get_records_sql("SELECT CONCAT(ww.userid,'_',ww.id) AS id, CONCAT(COALESCE(u.firstname,'Anonyme'),' ',COALESCE(u.lastname,'Anonyme')) AS user, ww.word, ww.timecreated FROM {wordcloud_words} ww LEFT JOIN {user} u ON (u.id = ww.userid) WHERE ww.wcid = ? AND ww.groupid = ? ORDER BY ww.word ASC, u.lastname ASC", array($this->activity->id, $groupid));
    ...
    private function update_modifiedtime() {
        global $DB;
        $DB->execute('UPDATE {wordcloud} SET timemodified = ? WHERE id = ?',array(time(), $this->activity->id));
    }

    private function normalize($value,$minvalue,$maxvalue) {
